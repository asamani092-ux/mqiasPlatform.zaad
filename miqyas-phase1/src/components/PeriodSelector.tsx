...
export default function PeriodSelector({ year, period }: { year: number; period: Period }) {
  ...
  return (
    <div className="period-selector">
      <select
        className="input-field period-selector__year"
        value={year}
        onChange={(e) => update(+e.target.value, period)}
        aria-label="السنة"
      >
      ...
      <select
        className="input-field period-selector__period"
        value={period}
        onChange={(e) => update(year, e.target.value as Period)}
        aria-label="الفترة"
      >
        {periods.map((p) => (
