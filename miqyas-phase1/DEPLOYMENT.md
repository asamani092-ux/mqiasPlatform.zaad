...
> **قاعدة بيانات قائمة (أُنشئت قبل baseline):** الـ migrations وُحّدت في `20260801000000_baseline` (يبني المخطط كاملًا من الصفر). إن كانت القاعدة تحمل المخطط مسبقًا (عبر `db push` أو الـ migrations القديمة)، علّم الـ baseline كمُطبَّق **مرة واحدة** قبل أول `migrate deploy`:
>
> ```bash
> npx prisma migrate resolve --applied 20260801000000_baseline
> ```
>
> القواعد الجديدة لا تحتاج هذا — `migrate deploy` يبنيها مباشرة.

> **حساب تجريبي (اختياري — للتطوير فقط):** إذا ضُبط `TEST_USER_PASSWORD` في `.env` قبل `npm run seed`، تُنشأ حساب `employee@zad.test` (دور موظف، قسم `1/1`) **مرة واحدة** إن لم يكن موجودًا. **لا تستخدم هذا في الإنتاج** — اترك المتغير فارغًا أو احذفه من `.env` على السيرفر الحي.

> **تقييم الأدوار (UAT):** صفحة `/uat` تظهر فقط عند `ENABLE_UAT=true`. في الإنتاج اترك `ENABLE_UAT=false` أو احذف المتغير. لتجربة تبويب «الأدوار والصلاحيات» أنشئ حسابًا واحدًا لكل دور عبر `/admin/users` (موظف، رئيس قسم، مدير إدارة، إدارة تنفيذية، مشرف نظام) ثم سجّل الدخول بكل حساب على حدة. التقييم في الذاكرة فقط أثناء الجلسة — بلا كتابة في قاعدة البيانات.
